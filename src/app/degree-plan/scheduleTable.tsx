...
export const columns: ScheduleTableColumn[] = [
...
const ScheduleTable: FC<ScheduleTableProps> = ({
   ...
   */
  const [scheduleRows, setScheduleRows] = useState<ScheduleTableRow[] | null>(null);
  ...
  const [previousRow, setPreviousRow] = useState<ScheduleTableRow | null>(null);
  ...
  */
   ...
   * 
   * If no schedule entries are found, simply set schedule rows
   * to one empty row.
   */
  ...
  const handleSelectRow: (
    ...
    const prevRows: ScheduleTableRow[] | null = rows ? rows : previousRow ? handleDeselect() : scheduleRows;
    if (!prevRows) {
      return;
    }
   ...
   * and returns.
   */
  const handleDeselect: () => ScheduleTableRow[] = useCallback(() => {
    if (!activeKey || !previousRow) {
      return scheduleRows || [];
    }
    
    let newRows: ScheduleTableRow[] = [];
  ...
  const filterCourses: (query: string) => CourseInfo[] = useCallback((query) => {
  ...
  /**
   * Removes from query dictionaries but also removes from grade values.
   * i.e. all instances of the key get deleted.
   */
  ...
  const addEmptyRow: (
    rows?: ScheduleTableRow[], 
    returnList?: boolean
  ) => ScheduleTableRow[] = useCallback((rows?, returnList?) => {
     ...
     */
    const emptyRow: ScheduleTableRow = { key: key, course_id: key };
  ...
  const handleSelectGrade: (key: string) => void = useCallback((key) => {
    ...
    let rows: ScheduleTableRow[] = scheduleRows!;
    if (activeKey) {
      rows = handleDeselect();
  ...
  const handleKeyDown: (
    ...
    if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') {
       ...
       * i.e rather than adding the currently active course on top of adding a new row.
       */
      return;
    }
  ...
  }, [activeKey, activeGradeKey, removeRealRow, addEmptyRow, incrementIndex, decrementIndex, decrementCol, incrementCol]);

  const handleClickOutside: (event: MouseEvent) => void = useCallback((event) => {
    if (tableRef && !tableRef.current!.contains(event.target as Node)) {
      handleDeselect();
      handleDeselectGrade();
    }
  }, [tableRef, handleDeselect, handleDeselectGrade]);
  ...
  const formatGradeInput: (item: ScheduleTableRow) => JSX.Element = useCallback((item) => {
    return (
      <TableCell
        className={`
  ...
  const nextToolTipDisplayContent: JSX.Element = useMemo(() => {
    ...
    const nonEmptyAndNonNan: ScheduleTableRow[] = scheduleRows.filter(
                ...
                {(columnKey) => {
                  ...
                  if (columnKey === 'course_id') {
                    ...
                    return (
                      <TableCell 
                        className={`
                          text-sm border-b border-gray-200 z-1
                          ${shouldBeHighlighted && 'bg-gray-100'}
                          ${disablePointerEvents && 'pointer-events-none'}
                        `}
                        onClick={() => handleSelectRow(item.key)}
                    ...
                    return (
                      <TableCell 
                        className={`
                          text-sm border-b border-r border-gray-200 z-1
                          ${shouldBeHighlighted && 'bg-gray-100'}
                          ${disablePointerEvents && 'pointer-events-none'}
                        `}
                        onClick={() => handleSelectRow(item.key)}
                    ...
                    return hasGradeValue && activeGradeKey !== item.key ? (
                      ...
                      <TableCell
                        className={`
                          ${(item.key === hoverGrade || activeGradeKey === item.key) && 'bg-gray-100'} 
                          ${disablePointerEvents && 'pointer-events-none'}
                          grade-cell border-b border-gray-200 z-1
                        `}
                        onClick={() => handleSelectGrade(item.key)}
                      >
                        {grade}
